...
app=app.replace('  function seriesResumeLabel(info,series){','  window.__debugSeriesProgressInfo=seriesProgressInfo;\n  function seriesResumeLabel(info,series){');
...
const state=window.__debugVedatorState,info=window.__debugSeriesProgressInfo(faq);
const progressKey=`episode-${completedEpisode}`;
console.log(JSON.stringify({completedEpisode,progressKey,progressRecord:state.progress[progressKey],progressKeys:Object.keys(state.progress),faqFirst:faq.episodes.slice(0,5),faqLength:faq.episodes.length,computedCompleted:faq.episodes.filter(number=>state.progress[`episode-${number}`]?.completed).length,functionCompleted:info.completed,functionTotal:info.total,functionRecord17:info.records.find(item=>Number(item.episode.number)===Number(completedEpisode))?.record,collectionKeys:Object.keys(state.collectionProgress)},null,2));
