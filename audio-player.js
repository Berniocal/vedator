(()=>{
  ...
  let restoreTarget=null,restoreTimer=0,isUserSeeking=false,allowBackwardSaveUntil=0;
  ...
  function updateSeekDisplay(usePreview=false){
    ...
    if(duration>0){seek.disabled=false;seek.max=String(Math.floor(duration));durationLabel.textContent=formatTime(duration)}
    else{seek.disabled=true;seek.max='1';durationLabel.textContent='–:––'}
    if(!isUserSeeking&&!usePreview){const value=Number.isFinite(audio.currentTime)?audio.currentTime:0;seek.value=String(Math.min(Number(seek.max)||1,Math.max(0,Math.floor(value))))}
  ...
  function decorateArticles(){
    document.querySelectorAll('#episodes article').forEach(article=>{
      ...
      if(!record||(!record.completed&&record.currentTime<10)){
        badge?.remove();article.classList.remove('vedator-listened','vedator-in-progress');play.textContent='Přehrát';return;
      }
      ...
      if(record.completed){
        badge.className='vedator-listen-status completed';badge.textContent='✓ Poslechnuto';
        article.classList.add('vedator-listened');article.classList.remove('vedator-in-progress');
        play.textContent=record.replaying&&record.currentTime>10?`Pokračovat znovu ${formatTime(record.currentTime)}`:'Přehrát znovu';
      }else{
        badge.className='vedator-listen-status in-progress';badge.textContent=`▶ Rozposloucháno${percent?` · ${percent} %`:''}`;
        article.classList.add('vedator-in-progress');article.classList.remove('vedator-listened');play.textContent=`Pokračovat ${formatTime(record.currentTime)}`;
      }
    });
  }

  function cancelAutomaticRestore(message='Automatické pokračování bylo zrušeno ručním posunem.'){if(restoreTarget===null)return;clearRestore();help.textContent=message}
  function resetAudio(save=true){
    ...
    try{audio.pause()}catch{}audio.removeAttribute('src');audio.load();currentKey='';currentTitle='';currentUrl='';lastSavedSecond=-1;isUserSeeking=false;
    seek.disabled=true;seek.value='0';currentTimeLabel.textContent='0:00';durationLabel.textContent='–:––';
  }
  function attemptPlay(session,shouldResume,record){
    const promise=audio.play();
    if(promise&&typeof promise.catch==='function')promise.catch(()=>{if(session!==currentSession)return;help.textContent=shouldResume?`Pozice ${formatTime(record.currentTime)} je připravena. Klepněte na přehrávání.`:'Klepněte na tlačítko přehrávání v přehrávači.'});
  }
  ...
  function openAudio(url,title){
    ...
    if(record?.completed&&!resumeReplay){
      ...
      persistProgress();decorateArticles();record=progress[key];allowBackwardSaveUntil=Date.now()+5000;
    }
    const shouldResume=Boolean(record&&record.currentTime>10&&(!record.completed||record.replaying));
    currentTitle=title;currentKey=key;currentUrl=url;lastSavedSecond=-1;
    barTitle.textContent=title;cardTitle.textContent=title;
    help.textContent=shouldResume?`Načítá se pokračování od ${formatTime(record.currentTime)}…`:'Pozice se ukládá do tohoto zařízení.';
    ...
    audio.onloadedmetadata=()=>{
      ...
      if(shouldResume&&Number.isFinite(audio.duration)&&audio.duration>0){
        ...
        try{audio.currentTime=target;seek.value=String(Math.floor(target));updateSeekDisplay(true)}catch{clearRestore();help.textContent='Uloženou pozici se nepodařilo načíst. Můžete se posunout ručně.'}
        restoreTimer=setTimeout(()=>{if(session!==currentSession||restoreTarget===null)return;clearRestore();help.textContent='Automatický posun trval příliš dlouho. Ruční posun je plně dostupný.'},4000);
      }else{clearRestore();help.textContent='Pozice se ukládá do tohoto zařízení.'}
    };
    audio.onerror=()=>{if(session!==currentSession)return;clearRestore();help.textContent='Zvuk se nepodařilo načíst. Zkuste epizodu zavřít a spustit znovu.'};
  ...
  function commitManualSeek(){
    if(seek.disabled||!currentKey)return;const target=Math.max(0,Math.min(Number(seek.max)||0,Number(seek.value)||0));
    cancelAutomaticRestore();allowBackwardSaveUntil=Date.now()+4000;try{audio.currentTime=target}catch{}isUserSeeking=false;currentTimeLabel.textContent=formatTime(target);help.textContent=`Posunuto na ${formatTime(target)}.`;
  }
  ...
  audio.addEventListener('seeked',()=>{if(restoreTarget!==null){const target=restoreTarget;clearRestore();help.textContent=`Pokračuje od ${formatTime(target)}. Pozice se průběžně ukládá.`}isUserSeeking=false;updateSeekDisplay();saveCurrentProgress(true)});
  ...
  document.addEventListener('click',event=>{const play=event.target.closest('.links .primary');if(!play)return;const url=play.getAttribute('href')||play.dataset.url;if(!url)return;event.preventDefault();event.stopPropagation();openAudio(url,episodeTitle(play))},true);
  const episodesBox=document.querySelector('#episodes');if(episodesBox)new MutationObserver(decorateArticles).observe(episodesBox,{childList:true});decorateArticles();
})();